...
OUTPUT_FILE="${NOTEBOOK_DIR}/01-Main-Dashboard.ipynb"

echo "--- Creating Main Surveillance Dashboard Notebook: '$OUTPUT_FILE' ---"

# Create the directory if it doesn't exist
mkdir -p "$NOTEBOOK_DIR"

# Use a 'here document' to write the multi-line JSON content into the file.
# The single quotes around 'EOF' are crucial to prevent the shell from
# interpreting any characters inside the block (like $, ", etc.).
cat << 'EOF' > "$OUTPUT_FILE"
{
 "cells": [
  {
...
echo "You can now run 'jupyter notebook' and open this file."